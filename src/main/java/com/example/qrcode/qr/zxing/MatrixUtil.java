...
final class MatrixUtil {
	...
	static void buildMatrix(BitArray dataBits,
			...
			int maskPattern,
			ByteMatrix matrix, 
			List<PositionalSquare> positionals) throws WriterException {
		clearMatrix(matrix);
		embedBasicPatterns(version, matrix, positionals);
...
//	static void buildDataMatrix(BitArray dataBits,
//			ErrorCorrectionLevel ecLevel,
//			Version version,
//			int maskPattern,
...
//	}
//
//	static void buildControlMatrix(BitArray dataBits,
//			ErrorCorrectionLevel ecLevel,
//			Version version,
//			int maskPattern,
//			ByteMatrix matrix) throws WriterException {
...
//	}
	...
	static void embedBasicPatterns(Version version, ByteMatrix matrix, List<PositionalSquare> positionals) throws WriterException {
		// Let's get started with embedding big squares at corners.
		embedPositionDetectionPatternsAndSeparators(matrix);
		// Then, embed the dark dot at the left bottom corner.
		embedDarkDotAtLeftBottomCorner(matrix);
		
		positionals.addAll(positionals(version, matrix));
	...
	public static List<PositionalSquare> positionals(Version version, ByteMatrix matrix){
		
		List<PositionalSquare> positionals = new ArrayList<PositionalSquare>();
		
		int pdpWidth = POSITION_DETECTION_PATTERN[0].length;
		
		// Left top corner.
		positionals.add(new PositionalSquare(0, 0, pdpWidth, 1, 1));
		// Right top corner.
		positionals.add(new PositionalSquare(matrix.getWidth() - pdpWidth, 0, pdpWidth, 1, 1));
		// Left bottom corner.
		positionals.add(new PositionalSquare(0, matrix.getWidth() - pdpWidth, pdpWidth, 1, 1));
		...
		for (int i = 0; i < numCoordinates; ++i) {
			for (int j = 0; j < numCoordinates; ++j) {
				...
				if (isEmpty(matrix.get(x, y))) {
					...
					// left top corner.
					positionals.add(new PositionalSquare(x - 2, y - 2, 5, 1, 1));
				}
			}
		}
